...
function renderScriptResult(value: unknown): { type: 'text'; text: string }[] {
  const v = value as { url: string; name: string; sha256: string; capabilities: string[]; resultJson: string; truncated: boolean }
...
const SCRIPT_RESULT_SCHEMA = {
  ...
  properties: {
    ...
    capabilities: { type: 'array' as const, required: true, items: { type: 'string' as const } },
    resultJson: { type: 'string' as const, required: true },
...
const RECIPE_STEP_SCHEMA = {
  ...
  additionalProperties: false,
  properties: {
    type: { type: 'string' as const, required: true, enum: ['wait', 'click', 'fill', 'type', 'press', 'select', 'check', 'hover', 'scroll', 'extract', 'assert', 'screenshot'] },
    condition: { type: 'string' as const, enum: ['selector', 'text', 'load', 'time'] },
    selector: { type: 'string' as const },
    value: { type: 'string' as const },
    text: { type: 'string' as const },
    key: { type: 'string' as const },
    timeoutMs: { type: 'number' as const },
    waitMs: { type: 'number' as const },
    deltaY: { type: 'number' as const },
    checked: { type: 'boolean' as const },
    mode: { type: 'string' as const, enum: ['text', 'html', 'links', 'attribute'] },
...
export function registerTools(ctx: Context, config: ResolvedConfig, service: BrowserService, assets?: AutomationAssetStore): void {
    ...
    output: {
      schema: {
        type: 'array', items: { type: 'object', additionalProperties: false, properties: {
          id: { type: 'string', required: true }, kind: { type: 'string', required: true }, status: { type: 'string', required: true },
          name: { type: 'string', required: true }, description: { type: 'string', required: true },
          domains: { type: 'array', required: true, items: { type: 'string' } }, tags: { type: 'array', required: true, items: { type: 'string' } },
          inputNames: { type: 'array', required: true, items: { type: 'string' } }, revision: { type: 'number', required: true },
          testStatus: { type: 'string', required: true }, successCount: { type: 'number', required: true }, failureCount: { type: 'number', required: true },
          updatedAt: { type: 'string', required: true }, lastRunAt: { type: 'string' },
        } },
      },
  ...
  register(defineTool({
    ...
    output: {
      schema: { type: 'object', additionalProperties: false, properties: {
        assetId: { type: 'string', required: true }, kind: { type: 'string', required: true }, resultJson: { type: 'string', required: true }, truncated: { type: 'boolean', required: true },
      } },
    ...
    output: {
      schema: {
        type: 'object', additionalProperties: false, properties: {
          url: { type: 'string', required: true }, title: { type: 'string' }, text: { type: 'string', required: true }, screenshotPath: { type: 'string' },
        },
      },
      render: (_args, value) => renderState(value as InteractiveState),
    },
  ...
  register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        const result = value as InteractiveState & { files: string[] }
  ...
  register(defineTool({
    ...
    output: {
      schema: {
        type: 'object', additionalProperties: false, properties: {
          url: { type: 'string', required: true }, resultJson: { type: 'string', required: true }, truncated: { type: 'boolean', required: true },
          capabilities: { type: 'array', required: true, items: { type: 'string' } }, warnings: { type: 'array', required: true, items: { type: 'string' } },
        },
      },
      render: (_args, value) => {
        const result = value as { url: string; resultJson: string; truncated: boolean; capabilities: string[]; warnings: string[] }
